...

{
    ...
    /// User joins a map - add to active users list
...

    {
...

        {
            ...
            // Store connectionId -> mapId mapping for disconnect handling
    ...
    /// User leaves a map - remove from active users and clear selection
    ...
    /// Update user's current selection on the map
...

    {
...

        {
            ...
            _logger.LogInformation(
                ...
                "User {UserId} selected {SelectionType}:{ObjectId} on map {MapId}",

                userId, request.SelectionType, request.SelectedObjectId, mapId);
    ...
    /// Get a specific user's current selection
    ...
    /// Get all active users and their selections on a map
...

        {
...

            {
...

                {
                    ...
                    JoinedAt = DateTime.UtcNow, // Could store this in Redis if needed
    ...
    /// Cleanup inactive selections (called by Hangfire job)
...

    {
...

        {
...

            {
                ...
                // If selection doesn't exist (expired), remove from active users
...
/// Cached user information for selections
