...
// {
//     public async Task<Option<ApprovalUrlResponse, Error>> CreateCheckoutAsync(decimal amount, string returnUrl,
...
//     }
//
//     public async Task<Option<ApprovalUrlResponse, Error>> CreateCheckoutAsync(ProcessPaymentReq request,
...
//
//             return Option.None<ApprovalUrlResponse, Error>(new Error("Payment.Stripe.CreateFailed",
//                 $"Failed to create Stripe session: {ex.Message}", ErrorType.Failure));
//         }
//     }
//
//     public async Task<Option<ConfirmPaymentResponse, Error>> ConfirmPaymentAsync(HttpRequest req, CancellationToken ct)
//     {
//         try
//         {
//             if (string.IsNullOrEmpty(sessionId))
//             {
//                 return Option.None<ConfirmPaymentResponse, Error>(new Error("Payment.Stripe.MissingSessionId",
//                     "Session ID or Payment ID is required for Stripe Checkout", ErrorType.Validation));
//             }
//
...
//             }
//
//             return Option.None<ConfirmPaymentResponse, Error>(new Error("Payment.Stripe.NotPaid",
//                 $"Payment not successful. Status: {session.PaymentStatus}", ErrorType.Validation));
//         }
//         catch (Exception ex)
//         {
//             return Option.None<ConfirmPaymentResponse, Error>(new Error("Payment.Stripe.Exception",
//                 $"Exception occurred: {ex.Message}", ErrorType.Failure));
//         }
//     }
//
//     public async Task<Option<CancelPaymentResponse, Error>> CancelPaymentAsync(CancelPaymentWithContextReq req,
...
//             if (string.IsNullOrEmpty(sessionId))
//             {
//                 return Option.None<CancelPaymentResponse, Error>(new Error("Payment.Stripe.MissingSessionId",
//                     "Session ID is required for Stripe Checkout", ErrorType.Validation));
//             }
//
//             var sessionService = new SessionService();
...
//                     new Error("Payment.Stripe.AlreadyPaid",
//                         "Payment has already been completed and cannot be cancelled", ErrorType.Validation));
//             }
//
